...
function PlayStore(search) {
   return new Promise(async (resolve, reject) => {
      try {
         const { data, status } = await axios.get(`https://play.google.com/store/search?q=${encodeURIComponent(search)}&c=apps`)
         const hasil = []
         const $ = cheerio.load(data)
...
function BukaLapak(search) {
   return new Promise(async (resolve, reject) => {
      try {
         const { data } = await axios.get(`https://www.bukalapak.com/products?from=omnisearch&from_keyword_history=false&search[keywords]=${encodeURIComponent(search)}&search_source=omnisearch_keyword&source=navbar`, {
            headers: {
...
async function stickersearch(query){
   return new Promise((resolve) => {
      axios.get(`https://getstickerpack.com/stickers?query=${encodeURIComponent(query)}`).then(({ data }) => {
         const $ = cheerio.load(data)
         const link = [];
...
function lirik(judul){
   return new Promise(async(resolve, reject) => {
      axios.get('https://www.musixmatch.com/search/' + encodeURIComponent(judul))
      .then(async({ data }) => {
         const $ = cheerio.load(data)
         const hasil = {};
